...
if(type==="client")body='<p class="lead">Add the contact and business details you want available inside Dillo HQ.</p><form data-type="client"><div class="form-grid"><div class="field"><label>Client name *</label><input name="name" required value="'+esc(item?.name)+'" ></div><div class="field"><label>Business name *</label><input name="company" required value="'+esc(item?.company)+'" ></div><div class="field"><label>Email</label><input name="email" type="email" value="'+esc(item?.email)+'" ></div><div class="field"><label>Phone</label><input name="phone" value="'+esc(item?.phone)+'" ></div><div class="field"><label>Website</label><input name="website" value="'+esc(item?.website)+'" ></div><div class="field"><label>Package</label>'+select("package",["Starter Presence","Consistency & Growth","Growth Partner"],item?.package)+'</div><div class="field"><label>Status</label>'+select("status",["Active","Onboarding","Paused"],item?.status||"Active")+'</div><div class="field full"><label>Notes</label><textarea name="notes" rows="3" >'+esc(item?.notes)+'</textarea></div></div>'+actions(type)+'</form>';
if(type==="project")body='<p class="lead">Set the owner, dates, status and current progress for this project.</p><form data-type="project"><div class="form-grid"><div class="field full"><label>Project name *</label><input name="name" required value="'+esc(item?.name)+'" ></div><div class="field"><label>Client</label><select name="client">'+companies+'</select></div><div class="field"><label>Status</label>'+select("status",["Not started","In progress","Review","Complete"],item?.status||"Not started")+'</div><div class="field"><label>Start date</label><input name="start" type="date" value="'+esc(item?.start)+'"></div><div class="field"><label>Due date</label><input name="due" type="date" value="'+esc(item?.due)+'"></div><div class="field"><label>Progress %</label><input name="progress" type="number" min="0" max="100" value="'+(item?.progress??0)+'"></div><div class="field full"><label>Notes</label><textarea name="notes" rows="3">'+esc(item?.notes)+'</textarea></div></div>'+actions(type)+'</form>';
if(type==="event")body='<p class="lead">Put meetings, deadlines and launch moments on your Dillo calendar.</p><form data-type="event"><div class="form-grid"><div class="field"><label>Title *</label><input name="title" required value="'+esc(item?.title)+'" ></div><div class="field"><label>Type</label>'+select("type",["Client call","Strategy","Deadline","Launch","Reminder","Other"],item?.type||"Client call")+'</div><div class="field"><label>Date *</label><input name="date" type="date" required value="'+esc(item?.date)+'"></div><div class="field"><label>Time</label><input name="time" type="time" value="'+esc(item?.time)+'"></div><div class="field"><label>Client</label><select name="client"><option value="">No client</option>'+companies+'</select></div><div class="field full"><label>Notes</label><textarea name="notes" rows="3">'+esc(item?.notes)+'</textarea></div></div>'+actions(type)+'</form>';
if(type==="invoice")body='<p class="lead">Record the invoice and keep its payment status current.</p><form data-type="invoice"><div class="form-grid"><div class="field"><label>Invoice # *</label><input name="number" required value="'+esc(item?.number||"")+'" ></div><div class="field"><label>Client</label><select name="client">'+companies+'</select></div><div class="field"><label>Amount *</label><input name="amount" type="number" min="0" step="0.01" required value="'+(item?.amount??"")+'" ></div><div class="field"><label>Status</label>'+select("status",["Pending","Paid","Overdue"],item?.status||"Pending")+'</div><div class="field"><label>Issue date</label><input name="issue" type="date" value="'+esc(item?.issue)+'"></div><div class="field"><label>Due date</label><input name="due" type="date" value="'+esc(item?.due)+'"></div><div class="field full"><label>Notes</label><textarea name="notes" rows="3">'+esc(item?.notes)+'</textarea></div></div>'+actions(type)+'</form>';
if(type==="file")body='<p class="lead">Choose a file from your computer. It will be stored securely in the Dillo cloud workspace.</p><form data-type="file"><div class="form-grid"><div class="field full"><label>Browse computer *</label><input name="upload" type="file" '+(id?"":"required ")+'accept="*/*"><small class="field-help">'+(id?"Choose a replacement file or leave this blank to keep the current file.":"Select the file you want to add to Dillo Files.")+'</small></div><div class="field full"><label>File name *</label><input name="name" required value="'+esc(item?.name||"")+'" ></div><div class="field"><label>Client</label><select name="client">'+companies+'</select></div><div class="field"><label>Category</label>'+select("category",["Brand assets","Content","Contract","Invoice","Brief","Deliverable","Other"],item?.category||"Brand assets")+'</div><div class="field"><label>Status</label>'+select("status",["Pending","Approved"],item?.status||"Pending")+'</div><div class="field"><label>Size</label><input name="size" value="'+esc(item?.size||"")+'"  readonly></div><div class="field full"><label>Notes</label><textarea name="notes" rows="3">'+esc(item?.notes)+'</textarea></div></div>'+actions(type)+'</form>';content.innerHTML='<h2 id="modalTitle">'+(id?"Edit ":"Add ")+heading+'</h2>'+body;back.hidden=false;back.style.display="grid";back.classList.add("is-open");document.body.style.overflow="hidden";const form=content.querySelector("form");content.querySelectorAll("[data-close]").forEach(b=>b.onclick=close);form.onsubmit=async e=>{e.preventDefault();const v=Object.fromEntries(new FormData(form));const upload=form.querySelector("input[type=file]")?.files?.[0]||null;if(type==="file"&&upload){v.name=upload.name;v.size=(upload.size/1024/1024).toFixed(1)+" MB"}delete v.upload;if(id){const x=state[key].find(x=>x.id==id);Object.keys(v).forEach(k=>x[k]=(k==="amount"||k==="progress")?Number(v[k]):v[k]);if(type==="file"&&upload)x.storage_path=await replaceCloudFile(x.storage_path,upload)}else{const x={id:Date.now(),...v};if(type==="project")x.progress=Number(v.progress)||0;if(type==="invoice")x.amount=Number(v.amount)||0;if(type==="file"){x.type=(v.name.split(".").pop()||"FILE").toUpperCase();if(upload)x.storage_path=await uploadCloudFile(upload)}state[key].push(x)}await saveCloudState();close();setView(type==="event"?"calendar":type+"s")}}
function settings(){content.innerHTML='<h2 id="modalTitle">Settings</h2><p class="lead">Dillo HQ is connected to Supabase for shared cloud data and private file storage.</p><div class="panel" style="box-shadow:none"><div class="panel-body"><b>Workspace data</b><p style="font-size:11px;color:#777;line-height:1.6">Clients, projects, calendar, invoices and file metadata are shared through the cloud workspace.</p><button class="btn secondary" id="resetData">Restore demo data</button></div></div>';back.hidden=false;back.style.display="grid";back.classList.add("is-open");document.body.style.overflow="hidden";document.getElementById("resetData").onclick=async()=>{if(confirm("Restore the original Dillo demo data? Your current browser data will be replaced.")){state=JSON.parse(JSON.stringify(seed));await saveCloudState();close();setView("overview")}}}
...
function profile(){content.innerHTML='<h2 id="modalTitle">Aya & Jam</h2><p class="lead">Admin workspace profile.</p><div class="list-row"><div><b>Dillo Socials</b><small>'+esc(currentUser?.email||"Authenticated user")+'</small></div><span class="tag green">Admin</span></div><div style="margin-top:14px"><button class="btn secondary" id="logoutBtn">Sign out</button></div>';back.hidden=false;back.style.display="grid";back.classList.add("is-open");document.body.style.overflow="hidden";document.getElementById("logoutBtn").onclick=async()=>{close();await sb.auth.signOut()}}
function close(){back.classList.remove("is-open");back.hidden=true;back.style.display="none";document.body.style.overflow=""}
async function remove(type,id){const key=type==="event"?"events":type+"s";const item=state[key].find(x=>x.id==id);if(type==="file")try{await deleteCloudFile(item?.storage_path)}catch(e){}state[key]=state[key].filter(x=>x.id!=id);await saveCloudState();setView(type==="event"?"calendar":type+"s")}
