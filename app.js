...
function settings(){content.innerHTML='<h2 id="modalTitle">Settings</h2><p class="lead">Dillo HQ is connected to Supabase for shared cloud data and private file storage.</p><div class="panel" style="box-shadow:none"><div class="panel-body"><b>Workspace data</b><p style="font-size:11px;color:#777;line-height:1.6">Clients, projects, calendar, invoices and file metadata are shared through the cloud workspace.</p><button class="btn secondary" id="resetData">Restore demo data</button></div></div>';back.hidden=false;back.style.display="grid";back.classList.add("is-open");document.body.style.overflow="hidden";document.getElementById("resetData").onclick=()=>{if(confirm("Restore the original Dillo demo data? Your current browser data will be replaced.")){state=JSON.parse(JSON.stringify(seed));await saveCloudState();close();setView("overview")}}}
